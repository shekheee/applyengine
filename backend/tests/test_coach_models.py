...
class CoachModelCatalogueTests(unittest.TestCase):
    def setUp(self):
        self.settings = Settings(
            ...
            openai_coach_models="gpt-5.6-terra,gpt-5.6-sol,gpt-5.5,gpt-5.6-luna",
    ...
    def test_default_models_exclude_unavailable_claude(self):
        ...
        self.assertTrue(
            {"gpt-5.6-terra", "gpt-5.6-luna", "gemini-3.1-pro-preview"}.issubset(ids)
        )
        self.assertFalse(any(model.startswith("claude-") for model in ids))

    def test_gpt_is_default(self):
        self.assertEqual(default_coach_model_id(self.settings), "gpt-5.6-terra")
    ...
    def test_model_catalogue_follows_fallback_order(self):
        ...
        self.assertEqual(available_coach_models(self.settings)[0].id, "gpt-5.6-terra")
