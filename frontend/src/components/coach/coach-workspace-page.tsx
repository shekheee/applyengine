...
function CoachWorkspaceInner({ initialMode }: { initialMode: CoachMode }) {
    ...
    <div
      className={cn(
        ...
        fullscreen
          ...
          : "h-[calc(100dvh-9rem)] min-h-[480px] lg:h-[calc(100dvh-2.5rem)]"
      )}
      data-coach-fullscreen={fullscreen ? "true" : undefined}
    >
      <CoachChat
        initialConversationId={initialConversationId}
        initialCoachMode={initialMode}
      ...
      <style jsx global>{`
        ...
        html.coach-fullscreen .app-chrome,
        html.coach-fullscreen .app-sidebar {
          display: none;
        }

        html.coach-fullscreen main,
        html.coach-fullscreen main > div {
        ...
        .coach-stream-cursor { animation: coach-stream-blink 1s step-end infinite; }
        .coach-typing-dot {
        ...
        .coach-prose :where(p, li) { color: var(--text-secondary); }
        .coach-prose :where(p + p) { margin-top: 0; }
        .coach-prose :where(p + ul, p + ol, ul + p, ol + p, pre + p, p + pre) {
          margin-top: 0.125rem;
        }
        .coach-prose :where(li + li) { margin-top: 0.125rem; }
        .coach-prose :where(li > ul, li > ol) { margin-top: 0.125rem; margin-bottom: 0; }
        .coach-prose pre,
        .coach-prose code { max-width: 100%; }
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .coach-stream-cursor,
          .coach-typing-dot { animation: none !important; opacity: 1; }
        }
      `}</style>
    </div>
  );
...
export function CoachWorkspacePage({ initialMode = "career" }: { initialMode?: CoachMode }) {
  return (
    <Suspense
      fallback={
        <div className="flex h-[70vh] items-center justify-center text-[var(--muted)]">
          Loading {initialMode === "buddy" ? "buddy" : "coach"}…
        </div>
      }
    >
      <CoachWorkspaceInner initialMode={initialMode} />
    </Suspense>
  );
}
